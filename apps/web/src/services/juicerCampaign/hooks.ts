...
import { FIRST_SQUEEZER_NFT_ABI, juicerCampaignAPI } from 'services/juicerCampaign/api'
import { JuicerProgress, NFTClaimRequest } from 'services/juicerCampaign/types'
...
export function useJuicerProgress() {
  ...
  useEffect(() => {
    ...
    window.addEventListener('juicer-campaign-updated', handleCampaignUpdate)
    return () => {
      window.removeEventListener('juicer-campaign-updated', handleCampaignUpdate)
    }
  }, [fetchProgress])
 ...
 * Detects ?juicer=true/false and manages localStorage
 * @internal
 */
function useUrlJuicerOverride(): boolean {
  const [overrideActive, setOverrideActive] = useState(() => {
    return localStorage.getItem('juicerOverride') === 'true'
  })

  useEffect(() => {
    const checkUrlParams = () => {
      const urlParams = new URLSearchParams(window.location.search)
      const juicerParam = urlParams.get('juicer')
...
export function useIsJuicerCampaignAvailable(): boolean {
  const account = useAccount()
  const isCampaignVisible = useIsJuicerCampaignVisible()
...
export function useTwitterFollow() {
  ...
  const startFollow = useCallback(async () => {
    ...
    try {
      ...
      window.dispatchEvent(new CustomEvent('juicer-campaign-updated'))
    } catch (err) {
...
export function useClaimNFT() {
  ...
  const claim = useCallback(async () => {
    ...
    try {
      ...
      const txHash = await juicerCampaignAPI.claimNFT(request, async (signature, contractAddr) => {
...
export function useIsEligibleForNFT(): boolean {
  const { progress } = useJuicerProgress()
