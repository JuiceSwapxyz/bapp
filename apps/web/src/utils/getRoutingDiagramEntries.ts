...
export default function getRoutingDiagramEntries(trade: ClassicTrade): RoutingDiagramEntry[] {
  // Callers may pass objects cast via `as ClassicTrade` where swaps is undefined at runtime
  return (
    // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition
    trade.swaps?.map(({ route: { path: tokenPath, pools, protocol }, inputAmount, outputAmount }) => {
      const portion =
