...
const TEST_TOKEN = {
  ...
  logoPath: '/Users/me/Downloads/taptap/taptap-logo-compressed.jpg',
}
...
test.describe('Launchpad: Create Token with MetaMask', () => {
  ...
  test('Create TapTap token on Launchpad', async () => {
    ...
    // Enable comprehensive browser console logging
    page.on('console', (msg) => {
      const text = msg.text()
    ...
    // Step 11: Handle MetaMask transaction confirmation using Synpress
    console.log('Waiting for MetaMask transaction popup...')
    await page.waitForTimeout(2000)

    // Use Synpress built-in method to confirm the transaction
    // This properly handles the MetaMask notification popup
    try {
      console.log('Confirming transaction with Synpress metamask.confirmTransaction()...')
      await metamask.confirmTransaction()
      console.log('Transaction confirmed via Synpress!')
      await screenshot(page, 'step14-tx-confirmed.png')
    } catch (e) {
      console.log('Synpress confirmTransaction failed, trying manual approach:', e)

      // Fallback: Try to find and click confirm button manually
      for (let attempt = 1; attempt <= 5; attempt++) {
        console.log(`[CreateToken] Manual attempt ${attempt}/5...`)
